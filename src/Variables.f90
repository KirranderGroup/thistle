...
!        ll2 = ll(apos)
    ...
    subroutine variables_resolved_cms(px,py,pz,ddx,ddy,ddz,zi,zj,e12,maxl,ngto,ng,group_start,group_count,group,ga,l,m,n,xx,yy,zz, &
        mmod,onerdm1,onerdm2,maxnmo,q,nq)
        ...
        real(kind=dp), intent(in),dimension(:),allocatable :: ga, xx, yy, zz, q
        real(kind=dp), intent(in),dimension(:,:),allocatable :: mmod
        real(kind=dp), intent(in),dimension(:,:)::onerdm1,onerdm2
        real(kind=dp), intent(out), dimension(ng,ng):: px,py,pz
        real(kind=dp), intent(out), dimension(ngto,ngto) :: zi,zj
        real(kind=dp), intent(out), dimension(nq,ng,ng) :: e12
        real(kind=dp), intent(out), dimension(maxl*2+1,maxl+1,maxl+1,ng,ng) :: ddx,ddy,ddz
        ...
        print*,maxval(mmod), maxval(onerdm1)
        counter=0
         do  ii=1,ngto
            ...
            do jj=1,ngto
                ...
                do cc=1,maxnmo
                     do cc2=1,maxnmo
                      ...
                      temp1=temp1+onerdm1(cc,cc2)*mmod(cc,ii)*mmod(cc2,jj)
                      temp1=temp1+onerdm1(cc,cc2)*mmod(cc,jj)*mmod(cc2,ii)

                      temp2=temp2+onerdm2(cc,cc2)*mmod(cc,ii)*mmod(cc2,jj)
                      temp2=temp2+onerdm2(cc,cc2)*mmod(cc,jj)*mmod(cc2,ii)
     ...
     do jj = 1, Ngto
       ...
       print*,maxval(e12)
    end subroutine variables_resolved_cms
