...
!            zcontrred2=zcontrred2/8.0
        ...
        do i=1,ncap
            do j=i+1,ncap
                do r=j+1,ncap
                    ...
                    tsi = tsi + 8.000 * f * e12(:, i, j) * e12(:, i, r)
                    count=count+1
    ...
    !$OMP PARALLEL do private(posI,posJ,spi,spj,spk,za,zb), &
        !$OMP& private(f,ii,jj,h,hx,hy,hz,i,j,dx1,dx2,dy1,dy2,dz1,dz2) shared(q,l,m,n, p0matrix), &
        !$OMP& shared( cutoffz,posits, cutoffmd,group_count,group_start,z) REDUCTION(+:tsi)
        do i=1,ncap
            do j=i+1,ncap
                    ...
                    allocate(posI(size(posits(i,:group_count(i)))),posJ(size(posits(j,:group_count(j)))))
                        ...
                        allocate(za(spi, spj), zb(spi, spj))
                    ...
                    tsi = tsi + 4.000 * f * e12(:, i, j) * e12(:, i, j)
                    count=count+1
...
!            zcontrred2=zcontrred2/8.0
    ...
    subroutine resolved_cms_integration(ncap,px,py,pz,l,m,n,p0matrix,dx,dy,dz,zi,zj,group_start,group_count,group, &
            cutoffz,cutoffmd, cutoffcentre,q,e12,tsi)
        ...
        real(kind=dp),dimension(:,:,:,:), intent(in) :: p0matrix
        REAL(kind=dp), intent(in), dimension(:,:,:) ::e12
        real(kind=dp), intent(in), dimension(:,:)::zi,zj
        ...
        print*,'posits allocated'
        tsi=0.0_dp
      !$OMP PARALLEL do private(posI,posJ,posK,posR,spi,spj,spk,spr,za,zb), &
       !$OMP& private(f,ii,jj,h,hx,hy,hz,i,j,k,r,dx1,dx2,dy1,dy2,dz1,dz2) shared(q,l,m,n, p0matrix), &
       !$OMP& shared( cutoffz, posits,cutoffmd,group_count,group_start,zi,zj) REDUCTION(+:tsi), &
       !$OMP& schedule(dynamic)
            ...
            do j=i+1,ncap
                do k=1,ncap
                    do r=k+1,ncap
                       ...
                       ! if(abs(f(1))>=1E-30)print*,f(1)
                        tsi = tsi + 4.000 * f * e12(:, i, j) * e12(:, k, r)
      ...
      !$OMP PARALLEL do private(posI,posK,posR,spi,spk,spr,za,zb), &
        !$OMP& private(f,ii,jj,h,hx,hy,hz,i,k,r,dx1,dx2,dy1,dy2,dz1,dz2) shared(q,l,m,n, p0matrix), &
        !$OMP& shared( cutoffz,posits, cutoffmd,group_count,group_start,zi,zj) REDUCTION(+:tsi), &
         !$OMP& schedule(dynamic)
        do i=1,ncap
...
!                    zcontrred2=zcontrred2
                    ...
                    tsi = tsi+ 2.000 * f * e12(:, i, i) * e12(:, k, r)
                    count=count+1
                    deallocate(posI,posK,posR,za,zb)
        ...
        print*,'second loop finished', tsi(1)

        !$OMP PARALLEL do private(posI,posJ,posK,spi,spj,spk,za,zb), &
        !$OMP& private(f,ii,jj,h,hx,hy,hz,i,j,k,dx1,dx2,dy1,dy2,dz1,dz2) shared(q,l,m,n, p0matrix), &
        !$OMP& shared( cutoffz,posits, cutoffmd,group_count,group_start,zi,zj) REDUCTION(+:tsi), &
         !$OMP& schedule(dynamic)
        do i=1,ncap
            do j=i+1,ncap
                do k=1,ncap
                    ...
                    allocate(posI(size(posits(i,:group_count(i)))) &
                                ,posJ(size(posits(j,:group_count(j)))),posK(size(posits(k,:group_count(k)))))
                    ...
                    allocate(za(spi,spj), zb(spk,spk))
...
!                    zcontrred2=zcontrred2
                    ...
                    tsi = tsi+ 2.000 * f * e12(:, i, j) * e12(:, k, k)
                    count=count+1
                    deallocate(posI,posJ,posK,za,zb)
        ...
        print*,'third loop finished', tsi(1)


          !$OMP PARALLEL do private(posI,posK,spi,spk,za,zb), &
        !$OMP& private(f,ii,jj,h,hx,hy,hz,i,k,dx1,dx2,dy1,dy2,dz1,dz2) shared(q,l,m,n, p0matrix), &
          !$OMP& shared( cutoffz, cutoffmd,posits,group_count,group_start,zi,zj) REDUCTION(+:tsi), &
         !$OMP& schedule(dynamic)
        do i=1,ncap
            do k=1,ncap
                ...
                tsi = tsi+ 1.000 * f * e12(:, i, i) * e12(:, k, k)
                deallocate(posI,posK,za,zb)
                count=count+1
               ! deallocate(dx1red, dy1red,dz1red,dx2red,dy2red,dz2red)
        ...
        print*,'last loop finished', tsi(1)

    end subroutine resolved_cms_integration



    SUBROUTINE set_P0_old(P0, lmax4, q)
    ...
    END SUBROUTINE set_P0_old


     SUBROUTINE set_P0_j2_old(P0, lmax4, q)
...
SUBROUTINE P_LMN(P0,l,m,n, q)
        ...
        P0(:,L+1,M+1,N+1)  = prod * (-1.0_dp)**(r/2) * q**r
...
SUBROUTINE P_LMN_j2(P0,l,m,n, q)
        ...
        P0(:,L+1,M+1,N+1)  = (l+m-2*n)/4.0_dp * prod * (-1.0_dp)**(r/2) * q**r
...
END SUBROUTINE P_LMN_j2
  ...
  SUBROUTINE P_LMN_old(P0, L, M, N, q)
...
elseif (L==0 .and. M==0 .and. N==16) then
    ...
    SUBROUTINE P_LMN_j2_old(P0, L, M, N, q)
...
SUBROUTINE BesselDeriv(BD, LL, MM,NN,a,b,c,LLmax)
    ...
    SUBROUTINE integral_ijkr_pzero(nq,l,m,n,gs,gc,p0mat,dx1,dy1,dz1,dx2,dy2,dz2,gi,gj,gk,gr,za,zb, &
        ...
        real(kind=dp), dimension(nq) :: prod7
                                        ...
                                        do mmp = 0, m(k)+m(r)
                                            ...
                                            do nnp = 0, n(k)+n(r)
                                                ...
                                                if (abs(prod6)>cutoff2) then
                                                    ...
                                                    prod7 = prod6 * P0mat(:,ll+llp+1,mm+mmp+1,nn+nnp+1)
                                                    F = F + prod7
...
!
        ...
        posi=posi+1
         ...
         !print*,h_saved(2)
    !    CALL BesselSum(F, mu, H, LLmax, h_saved)
        call bessels0rr(F,LLMAX+2,mu,H, h_saved)
...
!                    end if
    ...
    Subroutine bessels2(sum,order,mu,H, h_saved)
     ...
     Subroutine bessels2rr(h_sum,order,mu,H, h_saved)
!        INTEGER, PARAMETER :: dp = SELECTED_REAL_KIND(15)
!        INTEGER, PARAMETER :: ikind = SELECTED_INT_KIND(8)
!        integer:: order2
...
!        enddo
        ...
        if (order==2) then
            ...
            h_2=((3.d0/(Pmu)**3-1.d0/Pmu)*sinpmu-3.d0/(pmu)**2*cospmu)*H**(-2)
            h_sum=h_sum+h_saved(3)*h_2
        
        elseif(order==3) then

            h_3 = (pmu*(-15.0d0 + pmu**2.0d0)*cospmu + 3.0d0*(5.0d0 - 2.0d0*pmu**2.0d0)*sinpmu)/pmu**4.0d0*mu/H**3.0d0
            h_2=((3.d0/(Pmu)**3-1.d0/Pmu)*sinpmu-3.d0/(pmu)**2.0d0*cospmu)*H**(-2)    
            h_sum=h_sum+h_saved(3)*h_2+h_saved(4)*h_3
        
        elseif(order>=4) then
            call dphrec(pmu,allbessels,100,order+1,size(mu))
        
            do beta=2,order
                h_sum=h_sum+allbessels(beta+1,:)*mu**(beta-2)/H**beta*h_saved(beta+1)
            enddo
...
Subroutine bessels0rr(h_sum,order,mu,H, h_saved)
        ...
        integer(kind=ikind), intent(in) :: order
        real (kind=dp), dimension(:), intent(in) :: h_saved
        real (kind=dp), dimension(:), allocatable, intent(in) :: mu
        real (kind=dp),  intent(in) :: H
        real(kind=dp):: qrad,hfunc2,sumpmu,coeff,bess1,bess2,bessnew,time1,time2
        real(kind=dp),dimension(size(mu)):: pmu, muOH,h_2,h_1,h_0,h_r,hqr,bess,sinpmu,cospmu,invpmu
