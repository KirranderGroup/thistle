    ...
    do i=1,size(mat(:,1))
       ...
       subroutine one_rdm_bit(file_read,onerdm, maxnmo,numberlines,newdat,irep)
           ...
           do i=1,count
               ...
               read(15,*)j, civs(i), civs(i), Nalphbet(1,1,i), Nalphbet(1,2,i)
           enddo
           close(15)
...
!    close(15)
       ...
       subroutine one_rdm_bit_new(onerdm,civ1,civ2,Nalphbet, maxnmo,numberlines)
           ...
           integer(kind=ikind):: numberlines
           real(kind=dp),intent(in), dimension(numberlines) :: civ1,civ2
          ...
          do c1=1,numberlines
              ...
              do i=1,2
                  ...
                  buffer=Nalphbet(i,c1)
                  do while(buffer/=0_8)
                      j=trailz(buffer)

                      onerdm(j+1,j+1)=onerdm(j+1,j+1)+civ1(c1)*civ2(c1)
                      buffer=iand(buffer,buffer-1_8)
                  end do
          ...
          print*,'we have finished the elastic part'
           ...
           do c1=1,count-1
               ...
               do c2=c1+1,count
                    ...
                    diff1=popcnt(IEOR(Nalphbet(1,c1),Nalphbet(1,c2)))
                    diff2=popcnt(IEOR(Nalphbet(2,c1),Nalphbet(2,c2)))
                    newdiff=diff1+diff2
                    if (newdiff/=2) cycle

                    if (diff1==2) then
                            diff=IEOR(Nalphbet(1,c1),Nalphbet(1,c2))
                            porb=trailz(iand(Nalphbet(1,c1),diff))+1
                            qorb=trailz(iand(Nalphbet(1,c2),diff))+1

                            mylow=min(porb,qorb)
                            myhigh=max(porb,qorb)
                            ep=POPCNT(IAND(Nalphbet(1,c1),&
                                IAND(ibset(0_8,myhigh-1_8)-1_8,ibclr(-1_8,mylow)+1_8)))


                            onerdm(porb,qorb)=onerdm(porb,qorb)+civ1(c1)*civ2(c2)*phase_dbl(iand(ep,1_8))

                            onerdm(qorb,porb)=onerdm(qorb,porb)+civ1(c2)*civ2(c1)*phase_dbl(iand(ep,1_8))

                    end if

                   if (diff2==2) then
                            diff=IEOR(Nalphbet(2,c1),Nalphbet(2,c2))
                            porb=trailz(iand(Nalphbet(2,c1),diff))+1
                            qorb=trailz(iand(Nalphbet(2,c2),diff))+1

                            mylow=min(porb,qorb)
                            myhigh=max(porb,qorb)
                            ep=POPCNT(IAND(Nalphbet(2,c1),&
                                IAND(ibset(0_8,myhigh-1_8)-1_8,ibclr(-1,mylow)+1_8)))
...
!                   end if
       ...
       subroutine mcci_to_bit_3(file_read,file_write,numberlines)
           implicit none
           character(len=60), intent(in):: file_read,file_write
           integer(kind=SELECTED_INT_KIND(8)),intent(in):: numberlines
           integer*8:: icij(2,1,numberlines), j, k,i,nbft,itemp,ici,ntotal,phase,iperm
           integer*8, allocatable, dimension(:,:):: list
           real(kind=SELECTED_REAL_KIND(15)):: c1(numberlines), c2(numberlines), c3(numberlines)
           double precision, parameter :: phase_dbl(0:1) = (/ 1.d0, -1.d0 /)
           ...
           do i=1,numberlines
               read(15,*)ici, c1(i), c2(i), c3(i), icij(1,1,i), icij(2,1,i)
           enddo
           close(15)
           ...
           do ici=1,numberlines
               ...
               write(17,"(I15, 3E20.10, 2I20)")ici, c1(ici), c2(ici), c3(ici), icij(1,1,ici), icij(2,1,ici)
           ...
           end subroutine mcci_to_bit_3

        subroutine read_onerdm(filename,onerdm,maxnmo)
...
999             close(15)
        ...
        end subroutine read_onerdm



        subroutine read_twordm(filename,mat,total)
                ...
                integer(kind=ikind), intent(out), dimension(:,:), allocatable  :: mat
                real(kind=dp), intent(out), dimension(:), allocatable  :: total
                integer(kind=ikind) :: i,j,k,r,nlines
                real(kind=dp) :: val
...
999             close(14)
                ...
                allocate(mat(nlines,4))
                allocate(total(nlines))
