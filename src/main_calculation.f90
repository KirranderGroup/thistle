...
subroutine total_scattering_calculation(type,Zn,geom,state1,state2,state3,maxl,ngto,ng,ga,l,m,n,xx,yy,zz, &
        mmod,q,nq, group,&
...
!    use total_j0
 ...
 !   use elastic_j2
    ...
    implicit none
        ...
        integer(kind=ikind), intent(in):: ngto, ng,  nq, maxl,type, state1,state2,state3
        ...
        real(kind=dp),dimension(ngto,ngto):: z,zi,zj
        ...
        real(kind=dp),  dimension(:,:), allocatable :: onerdm_matrix,onerdm_matrix1,onerdm_matrix2
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       CASE(9)
            ...
            CALL set_P0(P0matrix, 4*maxval(l), q)
            call maxcoincidence(confs,ep3,ndiff2)
            call onerdm_creat(confs,civecs,onerdm_matrix1,nmomax,state1,state3)
            call onerdm_creat(confs,civecs,onerdm_matrix2,nmomax,state2,state3)
          

            open(unit=15,file='onerdm13.dat')
            do i=1,nmomax
                 write(15,*) onerdm_matrix1(i,5)
            end do
            close(15)

            open(unit=15,file='onerdm23.dat')
            do i=1,nmomax
                 write(15,*) onerdm_matrix2(i,5)
            end do
            close(15)    

            print*,'onerdm ayyy', maxval(onerdm_matrix)
            call variables_resolved_cms(px,py,pz,ddx,ddy,ddz,zi,zj,e12,maxl, ngto,ng,group_start,group_count,group,ga,l,m,n,xx,yy,zz, &
            mmod,onerdm_matrix1,onerdm_matrix2,nmomax,q,nq)
            print*,maxval(e12), 'and this is the maximum value'

            call resolved_cms_integration(ng,px,py,pz,l,m,n,p0matrix,ddx,ddy,ddz,zi,zj,group_start,group_count,group, &
                cutoffz,cutoffmd, cutoffcentre,q,e12,result2)
        ...
        mmod,q,nq, group,&
        cutoffz,cutoffmd,cutoffcentre,fileJ,fileJ_2,numberlines,newdat,start,end,start_2, end_2, fci,irep1,irep2,ordering1 &
                ,ordering2, readrdm,mcci,q_abs,result)
    ...
    implicit none
        ...
        INTEGER, PARAMETER :: dp = SELECTED_REAL_KIND(15)
        INTEGER, PARAMETER :: ikind = SELECTED_INT_KIND(8)
        logical, intent(in) :: fci,readrdm,mcci
        logical :: read2rdm2
        ...
        real(kind=dp),dimension(:,:,:),allocatable :: z1,z2,z1t,z2t
        real(kind=dp),dimension(ngto,ngto):: z,zi,zj
        real(kind=dp),dimension(numberlines):: civ1,civ2,civ3
        integer*8, dimension(2,numberlines) :: Nalphbet  
        character (len=100), intent(in) :: fileJ, fileJ_2
        ...
        real(kind=dp), dimension(ng,ng) :: px,py,pz
        real(kind=dp),  dimension(:,:,:,:), allocatable :: zcontr
        real(kind=dp),  dimension(:,:), allocatable :: onerdm_matrix,onerdm_matrix_1,onerdm_matrix_2
        real(kind=dp),  dimension(:), allocatable :: total,newtotal, totaldiv
        real(kind=dp),  dimension(nq,ng,ng) :: e12
        INTEGER(kind=ikind), DIMENSION(maxval(group))   :: group_start, group_count
        integer(kind=ikind), dimension(:), allocatable :: m1, m2, m3, m4
        integer(kind=ikind), dimension(:,:), allocatable :: mat,ep3,ndiff2
        integer(kind=ikind):: nmat,i,j,nmomax,LL,MM,NN,k,c1,sizenmat,spincont,counter,reads,fin,ini,npoints,ndivs,line
          ...
          !  call createtwordm(confs,civecs,ndiff2,ep3,mat,total)
            nmomax=15
                ...
                if (readrdm) then
                    ...
                    call read_twordm(fileJ_2,mat,total)
            ...
            print*,'Time 2rdm', time3-time2,numberlines
                ...
                npoints = size(mat(:,1))
                allocate(m1(npoints), m2(npoints), m3(npoints), m4(npoints))
                m1 = mat(:,1)
                m2 = mat(:,2)
                m3 = mat(:,3)
                m4 = mat(:,4)
                ...
                do i=1,nmat
                   print*, m1(i), m2(i), m3(i), m4(i),total(i)
                end do
                
                open(file='2rdm.txt', unit=15)
                 do i=1,nmat
                    write(15,*) m1(i), m2(i), m3(i), m4(i), total(i)
                 end do                
                nmomax=maxval(mat)
...
!              print*, 'yo there'
!              do j=1,nmat
!                print*, m1(j), m2(j), m3(j), m4(j), totaldiv(j)
!              end do
           call variables_total(px,py,pz,ddx,ddy,ddz,z1,z2,z1t,z2t, &
                   e12,maxl, ngto,ng,group_start,group_count,group,ga,l,m,n,xx,yy,zz, &
        mmod,m1,m2,m3,m4,nmat, total,q,nq)
           ...
           ! call maxcoincidence(confs,ep3,ndiff2)
            ...
            if (readrdm) then   
                 print*, 'yooooo'
                 call read_onerdm(fileJ,onerdm_matrix,nmomax)   
            ...
            open(unit=15,file='onerdm_el_bit.dat')
            do i=1,nmomax
                 write(15,*) onerdm_matrix(i,5)
            end do
            close(15)

            open(unit=15,file = 'onerdm.dat')

                        do i=1,nmomax                        
                            write(15,*) onerdm_matrix(:,i)
                        enddo
                
                close(15)
 
             print*,'rdm file created'
            call variables_elastic(px,py,pz,ddx,ddy,ddz,z,e12,maxl, ngto,ng,group_start,group_count,group,ga,l,m,n,xx,yy,zz, &
        mmod,onerdm_matrix,nmomax,q,nq)
            call elastic_integration(ng,px,py,pz,l,m,n,p0matrix,ddx,ddy,ddz,z,group_start,group_count,group, &
            ...
            if (readrdm) then   
                ...
                fileK = 'file.dat'
                call mcci_to_bit(fileJ,fileK,numberlines)
                call one_rdm_bit(fileK,onerdm_matrix,nmomax,numberlines,newdat,irep1)
            ...
            fileK = 'file.dat'
            call mcci_to_bit_3(fileJ,fileK,numberlines)
           
            print*, 'number of lines', numberlines
            open(file=fileK,unit=15)
            do i=1,numberlines
                read(15,*)line, civ1(i), civ2(i), civ3(i), Nalphbet(1,i),Nalphbet(2,i)
            end do
            close(15)
               
            print*,'calling onerdm_bit'
 
            call one_rdm_bit_new(onerdm_matrix_1,civ1,civ3,Nalphbet,nmomax,numberlines)
            call one_rdm_bit_new(onerdm_matrix_2,civ2,civ3,Nalphbet,nmomax,numberlines)
 
            call variables_resolved_cms(px,py,pz,ddx,ddy,ddz,zi,zj,e12,maxl, ngto,ng,group_start,group_count,group,ga,l,m,n,xx,yy,zz, &
            mmod,onerdm_matrix_1,onerdm_matrix_2,nmomax,q,nq)
