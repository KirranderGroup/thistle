program main
    ...
    character(len=60):: conf1, file_out,var,varrdm
    character(len=100)::file_bit, file_bit_2
    real(kind=dp), dimension(:), allocatable:: atoms
    integer(kind=ikind):: ngtos,norbs,nconfs,nstates,state1,state2,state3,natoms
...
!              close(15)
      ...
      Ngtos, ng, ga, l, m, n, xx, yy, zz, mmod, q, nq, group, cutoffz, cutoffmd, cutoffcentre,file_bit,file_bit_2, nconfs, newdat, &
    ...
    call total_scattering_calculation(Typec, atoms, geom, state1, &
            state2,state3, maxl, Ngtos, ng,ga, l, m, n, xx, yy, zz, mmod, q, nq, group, &
